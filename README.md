...
| 姓名  |           个人邮箱           |      UCSD邮箱      |                                                                                项目贡献                                                                                |
|:---:|:------------------------:|:----------------:|:------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| 吴俊霖 |  junlin-wu@foxmail.com   | juw029@ucsd.edu  | 项目搭建者，同时制定了项目规范。构建了项目以及确认了项目所使用的依赖包等。设计了路由路径。负责了PC端的页面框架搭建。同时也负责整合PC端其他人的代码，修改bug、调整样式与代码风格等。还负责了移动端页面搭建、设计与实现。同时通过实现了前端动态获取后端数据，从而将前端与后端结合到了一起。具体贡献可查询GitHub提交历史。 |
